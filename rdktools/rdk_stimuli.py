...
# note: for real time display, uncomment  self.clock.tick(self.tick_rate)
...
class Fixation(object):
    ...
    def show(self):
        ...
        while self.f_duration > 0:
            ...
            frame *= (1.0/frame.max())
            allframes[ii, :, :] = frame.astype('uint8')
            ii += 1
            self.f_duration -= 1
...
class BlankScreen(object):
    ...
    def __init__(self, display, duration=params.TIME_ITI,
                    win_width=params.WINDOW_WIDTH,
                    win_height=params.WINDOW_HEIGHT,
                    win_colour=params.WINDOW_COLOUR,
                    tick_rate=params.TICK_RATE):
...
class RandDot(pygame.sprite.Sprite):
    ...
    def move(self):
        if self.radius >= self.max_radius-5:
            self.reset_pos()
        self.x_0 += self.dx
        self.y_0 += self.dy
        self.rect.x += self.dx
